...
import com.orchestranetworks.addon.dint.template.SQLExportTemplateSpec;
 ...
 * 
 * <pre>{@code
 		<bean className="com.orchestranetworks.ps.scripttask.DataIntegrationExportSQLScriptTask">
		    <documentation xml:lang="en-US">
			<label>Data Integration Export SQL - using template </label>
			<description>
			    Script Task to Export data with a saved template using Data Integration add-on
			</description>
		    </documentation>
		    <properties>
			<property name="dataSpaceName" input="true">
				...
				<description>
				   The Source data space from where the Export SQL service is executed
				</description>
			    </documentation>
			</property>
			<property name="dataSetName" input="true">
				...
				<description>
				    The source data set where the Export SQL service is executed
				</description>
			    </documentation>
			</property>
...
public class DataIntegrationExportSQLScriptTask extends ScriptTaskBean
{
	...
	public void executeScript(ScriptTaskBeanContext context) throws OperationException
	{
		try
		{
			LoggingCategory.getWorkflow().info(
					"DataIntegrationExportSQLScriptTask started for {template id = " + templateId + ", source dataSpace = " + dataSpaceName + ", dataset = " + dataSetName + "}");
			...
			SQLExportTemplateSpec templateSpec = new SQLExportTemplateSpec(templateId, context.getSession());
			
			templateSpec.setDataset(dataSet); // source dataset
			DataIntegrationExecutionResults sqlImportResults = DataIntegrationExecutor.getInstance().execute(templateSpec);
			...
			LoggingCategory.getWorkflow().info("DataIntegrationExportSQLScriptTask completed....");

		}
		catch (DataIntegrationException e)
		{
			LoggingCategory.getWorkflow().error("DataIntegrationExportSQLScriptTask could not complete successfully. Exception : " + ExceptionUtils.getStackTraceAsString(e));
